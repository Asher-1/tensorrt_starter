...
void preProcess(uchar4 **h_inputImageRGBA, uchar4 **h_outputImageRGBA,
  ...
  *h_filter = new float[blurKernelWidth * blurKernelWidth];
  ...
  checkCudaErrors(
      cudaMemset(*d_blueBlurred, 0,
                 sizeof(unsigned char) *
                     numPixels));  // make sure the context initializes ok
  checkCudaErrors(cudaFree(0));
}
...
__global__ void gaussian_blur(const unsigned char *const inputChannel,
  ...
  for (int py = 0; py < filterWidth; py++) {
    for (int px = 0; px < filterWidth; px++) {
      ...
      color +=
          filter_value * static_cast<float>(inputChannel[c_y * numCols + c_x]);
    }
  }
...
int main(int argc, char *argv[]) {
  // load input file
  std::string input_file = "data/cinque_terre_small.jpg";
  // define output file
  std::string output_file = "data/cinque_terre_small_blur.jpg";
